    ...
    if (self.website.indexOf('https:') === 0) {
        ...
        req = http.request(options, function (res) {
            ...
            if (res.statusCode === 200) {
                ...
                res.on('end', function () {
                    // Website is up
                    //self.isOk(body, self.bodyContain, self.website, responseTime);
                    self.isOk({
                        body: body,
                        bodyContain: self.bodyContain,
                        website: self.website,
...
Monitor.prototype.isOk = function (obj) {
    if (obj.body.indexOf(obj.bodyContain) > 0) {
        ...
        this.setIsEventNeedTrigger(data);
...
Monitor.prototype.setIsEventNeedTrigger = function (data) {
    var nowDate = new Date;
    var secondsDiff = moment(nowDate).diff(this.lastTriggerTime, 'seconds')

    if (secondsDiff > config.sendMailInterval) {
        this.lastTriggerTime = new Date;
        data.isEventNeedTrigger = true;
    }
}
